...
2-3 | Ouvrir un fichier comportant des espaces | cat "file with spaces" |
3-4 | Se diriger dans un dossier et voir tous les fichiers, ceux cachés aussi | `cd`, `ll -a` | `cd` : change directory , j'aime utiliser `ll` directement car je trouve ça plus simple d'observer les fichiers en liste longue
...
5-6 | Trouver un fichier avec plusieurs propriétés connues ; taille, human-readable, not executable | `find` | `$ find inhere/ -type f -size 1033c ! -executable` <br> **inhere/** : Cherche dans le dossier "inhere" <br> **-type f** : Cherche uniquement des fichiers (pas des dossiers) <br> **-size 1033c** : Le c indique les bytes (octets) <br> **! -executable** : Le ! veut dire "PAS", donc on cherche ce qui n'est pas exécutable
...
12-13 | Travailler sur un fichier encodé/compressé étape par étape, en utilisant xxd, gzip, bzip2 et tar pour reconstituer un fichier ASCII lisible. | `mktemp -d`, `file`, `cp`, `cd`, `xxd -r data.txt data.bin`, `mv`, `gzip -d`, `bzip2 -d`, `tar -xvf`| Utiliser `file` pour identifier le type de chaque fichier, renommer les extensions si besoin (`.gz`, `.bz2`, `.tar`), puis décompresser successivement. <br> **Astuce** : travailler dans un dossier temporaire (`mktemp -d`) pour éviter d’altérer les fichiers d’origine. <br> Répéter jusqu’à obtenir un `ASCII text` contenant le mot de passe.
